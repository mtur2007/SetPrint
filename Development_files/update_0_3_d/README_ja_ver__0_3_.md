...
- ## `set_list` メソッド
   ...
   - #### 戻り値
    ...
    - ## **推奨設定例**
        ...
        **用途**: 次元要素毎で配列の次元違いを把握したい次元要素。

        **影響**: 後にx方向へ展開する際の、並行要素ともなります。
    ...
    - ## x 方向 - 配列の `次元の合致`
        ...
        `y方向` に展開されている並行な配列の `次元` との整合性を維持する為、<br>
        想定されていない異常な配列や、部分的な空間があった場合、発見がしやすくなります。
    ...
    - ## y 方向 - 配列の `順序の合致`
        ...
        `x方向` に展開されている並行な配列の `順序` との整合性を維持する為、<br>
        想定されていない異常な配列や、部分的な空間があった場合、発見がしやすくなります。
