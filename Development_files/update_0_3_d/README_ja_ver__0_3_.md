...
- ## `set_list` メソッド
   ...
   - #### 戻り値
    ...
    - ## **推奨設定例**
        ...
        **用途**: 次元要素毎で配列の欠落を把握したい次元要素。

        **影響**: y方向の並行要素ともなり、後の次元をy方向へ展開した場合、分割されて表示されます。
        ...
        **用途**: x方向の並行要素として配列の欠落を把握したい次元要素。

        **影響**: x方向の並行要素として、後の次元をx方向へ展開した場合、分割されて表示されます。
        ...
        **用途**: 指定した次元の配列をx方向の並行配列と指定し、<br>   　　  `順序の合致`と`次元の合致`を１行で簡潔にまとめて表示させたい場合。<br>
    ...
    `並行要素`と比較して、データが欠けている部分を自動で空白で埋めるという機能があります。
    
    具体的には `y軸` と `x軸` の重複にって表現されます。
    ...
    - ## y 軸 - 配列の `次元の合致`
        ...
        <br>`y`の場合、xの並行配列にする為、配列は次元の違いに関係なく、縦に表示されます。
    ...
    ### ※ 1, 並行要素との比較は直近の並行要素の展開方向の整合性が取られる為、<br>垂直な方向の整合性は並行要素単位でのものとなります。
